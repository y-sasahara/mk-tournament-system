@section('title', __('組分けツール（スネーク方式） - 組分け結果'))

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('組分けツール（スネーク方式） - 組分け結果') }}
        </h2>
    </x-slot>
    ...
    <div class="container md:max-w-7xl w-11/12 mx-auto mt-5 mb-5 p-8 border rounded-3xl border-gray-200 shadow bg-white min-h-screen">
        <h3 class="text-xl mb-4">{{ $snakeGrouping->name }} - {{ $snakeGrouping->getRoundText() }} 組分け結果</h3>
        <div>
            <p>作成者：{{ $snakeGrouping->user->name }}</p>
            <p>対戦形式：{{ $snakeGrouping->getTournamentTypeText() }}</p>
        </div>
        <div class="my-2">
        ...
        <span id="result">{!! nl2br(e($snakeGrouping->result)) !!}</span>
    </div>
    <script>
