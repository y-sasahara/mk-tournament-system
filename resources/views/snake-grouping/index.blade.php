@section('title', __('組分けツール（スネーク方式） - 組分け結果一覧'))

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('組分けツール（スネーク方式） - 組分け結果一覧') }}
        </h2>
    </x-slot>
    ...
    <div class="container md:max-w-7xl w-11/12 mx-auto mt-5 mb-5 p-8 border rounded-3xl border-gray-200 shadow bg-white min-h-screen">
        <div class="overflow-x-auto relative">
            <table class="border border-slate-300 whitespace-nowrap">
                ...
                <tbody>
                    @foreach ($snakeGroupings as $snakeGrouping)
                    @if (!$snakeGrouping->delete_reason)
                    <tr>
                    @else
                    <tr class="bg-gray-400">
                    @endif
                        <td class="px-4 py-2 border border-slate-300">{{ $snakeGrouping->name }}</td>
                        <td class="px-4 py-2 border border-slate-300">{{ $snakeGrouping->getRoundText() }}</td>
                        <td class="px-4 py-2 border border-slate-300">{{ $snakeGrouping->created_at->isoFormat('YYYY/MM/DD HH:mm') }}</td>
                        <td class="px-4 py-2 border border-slate-300">{{ $snakeGrouping->user->name }}</td>
                        <td class="px-4 py-2 border border-slate-300">{{ $snakeGrouping->delete_reason }}</td>
                        <td class="px-1 py-2 border-y border-slate-300 text-center">
                            <button type="button" class="mx-1 px-2 py-1 rounded bg-sky-400 text-white hover:bg-sky-500" onclick="location.href='{{ url('snake-grouping/'. $snakeGrouping->id) }}'">{{ __('表示') }}</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        {{ $snakeGroupings->links() }}
        @if (Auth::check() && Auth::user()->hasOrganizerRole())
        <button type="button" class="mt-4 px-2 py-1 w-fit rounded bg-sky-400 text-white hover:bg-sky-500" onclick="location.href='{{ url('snake-grouping/create') }}'">{{ __('組分けデータを作成する') }}</button>
        @endif
        <div class="mt-5">
